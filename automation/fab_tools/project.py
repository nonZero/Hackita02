...
def deploy(restart=True, quick=False):
    # if restart:
    #     sudo("supervisorctl stop webhackita02_celery")
    with virtualenv(env.code_dir):
        run("git pull")
        if not quick:
    ...
    if restart:
        reload_app()
        # sudo("supervisorctl start webhackita02_celery")
...
@task
def create_venv():
    run("virtualenv -p python3.4 {} --prompt='({}) '".format(env.venv_dir, env.user))
    upgrade_pip()
...
def create_local_settings():
    with cd(env.code_dir):
        upload_template('conf/prod_settings_template.py',
                        env.code_dir + 'hackita02/local_settings.py',
                        {
                            'webuser': env.webuser,
                            'host': env.vhost,
